...
void Gimbal::send(const TxPacket & pkt)
{
  TxPacket tx = pkt;
  tx.crc16 = tools::get_crc16(
    reinterpret_cast<uint8_t *>(&tx), sizeof(tx) - sizeof(tx.crc16));

  try {
    serial_.write(reinterpret_cast<uint8_t *>(&tx), sizeof(tx));
  } catch (const std::exception & e) {
    tools::logger()->warn("[Gimbal] Failed to write serial: {}", e.what());
  }
}
...
void Gimbal::read_thread()
{
    ...
    fmt::print("yaw data: {:.3f}, pitch data: {:.3f}\n", state_.yaw, state_.pitch);
